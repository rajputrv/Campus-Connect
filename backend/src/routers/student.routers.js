...
const {registerEmaildata, welcomeEmaildata} = require("../utils/registerEmaildata.js");
  ...
  handler(async (req, res, next) => {
    try {
     ...
     // console.log(req.body);
      ...
      console.log('CreateStudentt =>',CreateStudent);

      sendMail(email, "Verify your acccount,", registerEmaildata(name, email, password, CreateStudent.verificationCode));
  ...
  "/verifyEmail",
   handler(async (req, res, next) => {
    try {
      ...
      const student = await Student.findOne({email:address, verificationCode: otp});
      if (!student) {
        return next(
          errorHandler(404, "Either no stuent exists with this email or verification code is wrong")
        );
      }  
      sendMail(student.email, "Welcome to the Edu♾️onnect", welcomeEmaildata(student.name, student.email));
